...
//    String hubHostAndPort="http://lhrvms-aplqv002.uk.ad.ba.com:5555/";
    String hubUrl="http://lhrvms-aplqv001.uk.ad.ba.com:5555/wd/hub/";
...
//        return responseBody;
        ...
        String responseBody;
        String selNodeHostName=null;
        String uri="grid/api/testsession?session=";

        CloseableHttpClient client = HttpClientBuilder.create().build();
        HttpGet request = new HttpGet(hubHostAndPort +uri+sessionID);
        request.addHeader("User-Agent", USER_AGENT);
        HttpResponse response = client.execute(request);
        responseBody = EntityUtils.toString(response.getEntity());
        ...
        if(m.find()){
            selNodeHostName = m.group(1);
        }
        return selNodeHostName;
    }
